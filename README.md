...
- 이번 영수증 글자 검출 대회에 참여하면서 저는 큰 한계를 실감했습니다. 기존에 제공 받은 baseline의 세팅부터 복잡한 이미지 내 텍스트를 효과적으로 학습하고 분류하기 위해 Convolutional Neural Networks(CNN)와 같은 심층 신경망을 사용하는 과정 등에서 많은 시행착오가 있었습니다. 이번 경험은 이론적 지식을 실제 문제에 적용하는 과정의 중요성을 깊이 깨닫게 해주었습니다. 데이터 전처리의 복잡성과 모델 최적화 과정의 어려움을 직접 겪으면서, 이론과 실제의 괴리를 줄이기 위한 노력의 필요성을 느꼈습니다. 또한, 이론적으로 배운 내용을 실제로 어떻게 활용해야 하는지에 대한 인사이트를 얻는 중요한 계기가 되었습니다. 앞으로는 이러한 경험을 바탕으로 머신러닝 및 딥러닝 분야에서 실력을 더욱 키우고, 프로그래머로서 성장하기 위해 적극적으로 노력할 계획입니다. 코딩 실력을 강화하고 다양한 툴을 활용하는 법을 배우며, 계속해서 새로운 모델을 실험해 볼 것입니다. 이론과 실전이 결합된 체계적이고 효과적인 접근 방식을 개발해 나가면서, 향후 유사한 대회나 프로젝트에서 보다 나은 성과를 달성할 수 있기를 기대합니다.

### Meeting Log

- 저희 조는 대학원 및 직장인 팀으로서 모임을 가질 수가 없었기에 각자가 결과물을 제출하게 되었습니다. 감사합니다.
